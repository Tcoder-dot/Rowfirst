...
def _analysis_request(text: str) -> tuple[dict[str, Any], dict[str, str | bool]]:
    topic_match = re.search(r"(?im)^\s*topic\s*:\s*(.+?)\s*$", text or "")
    ho_match = re.search(r"(?im)^\s*h(?:0|o)\s*:\s*(.+?)\s*$", text or "")
    data_lines = []
    for line in (text or "").splitlines():
        if re.match(r"(?i)^\s*(topic|h(?:0|o))\s*:", line):
            continue
        data_lines.append(line)
    metadata: dict[str, str | bool] = {
        "topic": topic_match.group(1).strip() if topic_match else "",
        "ho": ho_match.group(1).strip() if ho_match else "",
